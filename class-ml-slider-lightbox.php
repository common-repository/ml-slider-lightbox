...
class MetaSliderLightboxPlugin
{
    ...
    public function check_dependencies()
    {
        ...
        if (! $has_active_lightbox || ! $this->check_if_plugin_is_active('MetaSlider')) {
            ...
            add_action('metaslider_admin_notices', array($this, 'show_dependency_warning'), 10, 3);
            return false;
        }
        ...
        if ($active_plugin_count > 1) {
            ...
            // @since 1.13.2 - Notices in MetaSlider admin pages
            add_action('metaslider_admin_notices', array($this, 'show_multiple_lightbox_warning'), 10, 3);
            return false;
        }
    }
        ...
        if (! isset($active_lightbox_data['Version'])) {
            return array_merge($aFields, array(
                'lightbox' => array(
                    ...
                    'value' => '',
                    'after' => sprintf(
                        "%s <a href='https://wordpress.org/plugins/ml-slider-lightbox#description-header' target='_blank'>%s</a>",
                        __(
                            "Warning: MetaSlider Lightbox is installed but no supported lightbox plugin is active.",
                            "ml-slider"
                        ),
                        __("More info", "ml-slider-lightbox")
                    )
                )
            ));
        }
